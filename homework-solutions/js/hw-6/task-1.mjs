...
for (const number of numbers) {
  if (number % 2 === 0) {
    forOf.push(number * number);
  } else {
    forOf.push(number * number * number);
  }
}
/*
2. Методы массивов
...
fiveNumbers.push(6);
fiveNumbers.unshift(0);
fiveNumbers.splice(2, 1);
fiveNumbers.pop();
result = fiveNumbers;
...
let seconArray = [6, 7, 8, 9, 10];
let mergedArray = [...firstArray, ...seconArray];
